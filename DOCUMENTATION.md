...
```bash
git clone git@github.com:pact-foundation/blog.pact.io.git
cd blog.pact.io
```
...
On each deployment, the Heroku Node/npm build process will **auto-upgrade Ghost to the newest 1.x version**. To prevent this behavior, use npm 5+ to create a lockfile.
